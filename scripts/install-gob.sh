#!/bin/bash
# GOB Installation Script - Can be curl-piped for one-liner install
# Usage: bash <(curl -s https://raw.githubusercontent.com/dusty-schmidt/gob-01/main/scripts/install-gob.sh)
...
echo -e "${BLUE}"
echo "╔═══════════════════════════════════════╗"
echo "║  GOB - Gradient Observation Bridge    ║"
...
if [ ! -d "$INSTALL_DIR" ]; then
    echo -e "${BLUE}Cloning GOB repository...${NC}"
...
# Run complete setup
echo -e "\n${BLUE}Running complete setup...${NC}\n"
